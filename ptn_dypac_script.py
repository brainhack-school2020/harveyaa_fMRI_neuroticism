...
if __name__ == "__main__":
    parser = ArgumentParser()
     #'./HCP_PTN1200_recon2/NodeTimeseries_3T_HCP1200_MSMAll_ICAd200_ts2/node_timeseries/3T_HCP1200_MSMAll_d200_ts2'
    parser.add_argument("--ts_dir",help="Path to time series directory",dest='ts_dir') 
    args = parser.parse_args()
    
    for filename in os.listdir(args.ts_dir):
        path = os.path.join(directory, filename)
        series = np.loadtxt(path.format(sub))
        dypac = Dypac()
        dypac.fit(series)
        #WRITE RESULT OF DYPAC OUT
        dypac.components_ #stab_maps (nd array size n_states?)
        dypac.dwell_time_ #dwell_time (nd array)
        dypac.embedding #Embedding object
